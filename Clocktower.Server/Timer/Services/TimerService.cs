...
public class TimerService(ITimerCoordinator timerCoordinator, IGamePerspectiveService gamePerspectiveService) : ITimerService
{
    ...
    {
        if (!gamePerspectiveService.GameExists(gameId)) return Result.Fail<TimerState>(Errors.GameNotFound(gameId));
