...
public class TimerService(ITimerCoordinator timerCoordinator, IGamePerspectiveStore gamePerspectiveStore) : ITimerService
{
    ...
    {
        if (!gamePerspectiveStore.GameExists(gameId)) return Result.Fail<TimerState>(Errors.GameNotFound(gameId));
