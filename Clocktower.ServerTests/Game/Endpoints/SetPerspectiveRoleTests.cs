...
public class SetPerspectiveRoleTests
{
    ...
    public void Map_RegistersCorrectly()
    {
        ...
        builder.GetEndpoint("/{gameId}/set-perspective-role/{targetUserId}/{roleId?}")
    ...
    public async Task Handle_ReturnsBadRequest_WhenServiceReturnsFalse()
    {
        ...
        var request = new SetPerspectiveRole.Request(CommonMethods.GetRandomString(), CommonMethods.GetRandomSnowflakeStringId(), CommonMethods.GetRandomString());
        var error = Result.Fail<string>(ErrorKind.Invalid, "error code", "error message");

        _mockGameService.Setup(o => o.SetPerspectiveRole(request.GameId, userId, request.TargetUserId, request.RoleId)).ReturnsAsync(error);

        var result = await SetPerspectiveRole.Handle(user, request, _mockGameService.Object);

        _mockGameService.Verify(o => o.SetPerspectiveRole(request.GameId, userId, request.TargetUserId, request.RoleId), Times.Once);
    ...
    public async Task Handle_ReturnsOk_WhenServiceReturnsTrue()
    {
        ...
        _mockGameService.Setup(o => o.SetPerspectiveRole(request.GameId, userId, request.TargetUserId, request.RoleId)).ReturnsAsync(success);
